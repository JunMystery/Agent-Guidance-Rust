...
[![Version](https://img.shields.io/badge/Version-v1.3.1-blue.svg)](Cargo.toml)
...
2. **Multi-Session & Multi-IDE Isolation**:
...
3. **Automatic 30-Day Session Garbage Collection (GC)**:
...
4. **Smart Skills Calls (Local ML Vector Search)**:
...
- **Hard Edit Gate (`workflow_gate action="authorize_edit"`)**: Code modification is BLOCKED until `plan_approved = true` and the agent explicitly declares a valid `architecture_pattern`:
  - `Clean_Architecture`
...
| `workflow_gate` | **Manager Gate**: Manages stage transitions (`check`, `status`, `set_stage`, `advance`, `authorize_edit`). | `action` | Composite `advance`, architecture enforcement, circuit breaker. |
