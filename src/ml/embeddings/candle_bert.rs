//! Candle BERT FP32 Embedding Engine Baseline
//!
//! Provides pure-Rust CPU, CUDA, and Apple Metal embedding inference
//! using HuggingFace's Candle library.
...
use super::model::resolve_repo_paths;

pub struct CandleBertInner {
    pub model: BertModel,
    pub tokenizer: Tokenizer,
    pub device: Device,
}

impl CandleBertInner {
    pub fn load_or_download() -> Result<Self> {
        ...
        info!("CandleBertInner loaded successfully on {}.", dev_name);
        Ok(Self {
            model,
            tokenizer,
            device,
    ...
    pub fn device_name(&self) -> &'static str {
        match self.device {
            ...
            Device::Cuda(_) => "Candle NVIDIA CUDA",
            Device::Metal(_) => "Candle Apple Metal",
        }
    }
    ...
    pub fn embed_batch(
        ...
        for chunk in texts.chunks(chunk_size) {
            ...
            let input_ids =
                Tensor::from_vec(batch_input_ids, (b_size, max_len), &self.device)?;
            let attention_mask =
                Tensor::from_vec(batch_attention_mask, (b_size, max_len), &self.device)?;
            let token_type_ids = input_ids.zeros_like()?;
