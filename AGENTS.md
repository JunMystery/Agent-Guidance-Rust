...
4. **Per-File Edit Authorization Gate, Modularity & 300 LOC Hard Cap**: Always call `workflow_gate(action="authorize_edit", project_path="...", relative_path="<exact_file_path>", risk_level="LOW", justification="...", architecture_pattern="Auto")` individually for EACH file BEFORE creating or modifying it. All source code files MUST remain strictly < 300 LOC (target < 150 LOC per sub-module; refactor bypass keywords: `refactor`, `decompose`, `extract`, `split`, `tách file`; exempt: docs, markdown, data, configs, assets). Strict Single Responsibility: compound plural suffixes (e.g. `*services`, `*controllers`, `*utils` - 40 patterns) and multi-component justifications are prohibited (`COMPOUND_FILE_NAME_PROHIBITED`, `MULTI_COMPONENT_NEW_FILE_PROHIBITED`).
