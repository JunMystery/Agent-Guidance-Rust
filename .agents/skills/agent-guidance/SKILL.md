...
2. If specific domain skills or coding standards are required, query them via `guidance(operation="search", query="...")`. When skills are proposed, ask user via `ask_question(questions=[{question: "...", options: [...], is_multi_select: true}])` before loading them via `select_skills(skills=[...], user_confirmed=true)`.
